...
{
echo "<html><head><title>GnuTheme help</title>"
echo "<meta name=\"generator\" content=\"gnutheme html_helpfiles.sh\">"
echo "</head><body>"
echo "<h1>GnuTheme help</h1>"
echo "Services"
echo "<ul>"
for d in $helpdir/*; do
	[ -d "$d" ] || continue
	...
	{
		echo "<html><head><title>GnuTheme help - $service</title>"
		echo "<meta name=\"generator\" content=\"gnutheme html_helpfiles.sh\">"
		echo "</head><body>"
		echo "</head><body>"
		echo "<h1>$service</h1>"
